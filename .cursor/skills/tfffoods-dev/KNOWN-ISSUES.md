...
- **Fix:** The hook now wraps `useSWR(session ? "/api/wishlist" : null)`, so every consumer shares one cache entry and one request; `toggleWishlist` writes the route's returned list straight into that cache (`mutate(..., { revalidate: false })`), which also fixed buttons not reflecting each other's state. Errors toast with a fixed `id` so N subscribers produce one toast. Public API (`wishlist`, `loading`, `error`, `toggleWishlist`, `isInWishlist`, `refreshWishlist`) is unchanged, so no component needed touching. Also removed a dead `wishlistItems` destructure on `/products` (the hook never returned that key) and an unused import on `/profile`.
- **Note:** a slow, blank page with "Compiling…" in the corner is just the dev compiler rebuilding an edited route, not a hang — check `npm run build` if in doubt.

### 2026-09-07 — "Failed to save list" after adding a field to an existing model
- **Symptom:** Saving a quick-order list toasted "Failed to save list" straight after `orderTemplates` was added to `utils/models/User.ts`. Lints clean, route logic correct.
- **Root cause:** Models are exported as `mongoose.models.User || mongoose.model("User", schema)` and `ensureModelsAreRegistered()` is a no-op, so `mongoose.models` survives Next's module reload. **A dev server started before a schema change keeps the old schema.** The new path did not exist, so `user.orderTemplates.push(...)` threw on `undefined`. Worse, an atomic `$push` would not have thrown at all — Mongoose strict mode silently strips unknown paths from updates, so writes vanish with a 200.
- **Fix:** **Restart the dev server after editing any Mongoose schema.** (The same applies to the later `orderTemplates.displayNames` addition — `schemaMissing()` now probes `displayNames.en`, so a stale server returns the explicit 503 instead of silently dropping bilingual names.) The route now guards with `User.schema.path("orderTemplates")` and returns 503 with an explicit "restart the dev server" message instead of failing silently, writes atomically (`$push` / `$set` with positional `$` / `$pull`) rather than calling `save()` on a projected document, and returns a `detail` field that the client toast displays instead of a generic message.
